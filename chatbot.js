...
[농장 정보 및 데이터]
- 오이 종류 :
  * 특품 오이: 곧고 굵기가 일정한 선물용/생식용 최상급 오이. 
  * 상품 오이: 준수한 모양의 가정용 오이무침, 반찬용. 
  * 공품 오이 (못난이): 모양이 휘었거나 사이즈가 일정하지 않지만 맛은 특품과 동일한 실속형. (개수 주문 불가)
  * 한입 오이 (시그니처): 등산/나들이, 샐러드 등 생식으로 껍질채 먹기 좋은 연하고 어린 오이(개수 주문 불가, 한입오이만 주문시 최대 6kg까지 가능)

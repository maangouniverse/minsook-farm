...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const orderMemoSelect = document.getElementById('orderMemoSelect');
  ...
  if (orderMemoSelect && orderMemo) {
    orderMemoSelect.addEventListener('change', () => {
      if (orderMemoSelect.value === '직접 입력하기') {
        orderMemo.style.display = 'block';
        orderMemo.value = '';
        orderMemo.focus();
      } else {
        orderMemo.style.display = 'none';
        orderMemo.value = orderMemoSelect.value;
      }
      calculateOrder();
    });
  }
  ...
  // Price policies per grade (populated dynamically from DB)
  const priceConfig = {};
  ...
  if (btnAddProduct && productSelect) {
    btnAddProduct.addEventListener('click', () => {
      ...
      if (!selectedGrade) {
        showToast('구매하실 품목을 선택 후 추가하기를 눌러주세요!');
        return;
      }
    ...
    for (const g in orderState) {
      ...
      if (qty > 0 && priceConfig[g]) {
        ...
        if (priceConfig[g].isQty) {
          const stepVal = priceConfig[g].step || 10;
          totalOriginalPrice += (qty / stepVal) * priceConfig[g].price;
        } else {
  ...
  function updateSMSPreview(origPrice = 0, discRate = 0, discAmount = 0, shipFee = 0, finalTotal = 0) {
    ...
    for (const g in orderState) {
      ...
      if (qty > 0 && priceConfig[g]) {
        ...
        let itemPrice = 0;
        if (config.isQty) {
          const stepVal = config.step || 10;
          itemPrice = (qty / stepVal) * config.price;
        } else {
    ...
    if (!itemsText) {
      itemsText = ' - 선택된 품목이 없습니다. 계산기에서 수량을 추가해 주세요.\n';
    }
    ...
    if (orderType === 'delivery') {
      ...
      if (orderMemoSelect) {
        if (orderMemoSelect.value === '직접 입력하기') {
          memo = (orderMemo && orderMemo.value) ? orderMemo.value.trim() : '';
        } else {
          memo = orderMemoSelect.value;
        }
      } else {
        memo = (orderMemo && orderMemo.value) ? orderMemo.value.trim() : '';
      }
...
※ [카카오톡으로 주문서 전송하기] 버튼을 누르시면 주문이 접수됩니다.`;
  ...
  async function fetchProductsAndInit() {
    ...
    if (window.location.protocol === 'file:') {
      ...
      products = JSON.parse(localStorage.getItem('minsook_products') || '[]');
      if (products.length === 0) {
        products = [
          { id: 1, name: '특품 오이 (무게)', description: '곧고 굵기가 일정하며 겉 표면의 돌기가 선명합니다. 아삭한 맛이 일품이라 생식용, 샐러드용, 그리고 고급 선물용으로 강력 추천합니다.', image_url: 'minsook_main.jpg', price: 7900, unit: 'kg', step: 1.0, badge_text: '최상급 선물용', badge_class: 'badge-special', is_bite: 0, is_qty: 0 },
          { id: 2, name: '특품 오이 (개수)', description: '곧고 굵기가 일정하며 겉 표면의 돌기가 선명합니다. 10개 단위로 실속 있게 구매할 수 있습니다.', image_url: 'minsook_main.jpg', price: 7500, unit: '개', step: 10.0, badge_text: '최상급 선물용', badge_class: 'badge-special', is_bite: 0, is_qty: 1 },
          { id: 3, name: '상품 오이 (무게)', description: '모양과 굵기가 준수하며 일상적인 식탁에 어울리는 등급입니다. 무침, 냉국, 피클 등 가벼운 반찬거리를 만드는 가정용으로 제격입니다.', image_url: 'minsook_detail_2.jpg', price: 5900, unit: 'kg', step: 1.0, badge_text: '가정용 추천', badge_class: 'badge-good', is_bite: 0, is_qty: 0 },
          { id: 4, name: '상품 오이 (개수)', description: '모양과 굵기가 준수하며 일상적인 식탁에 어울리는 등급입니다. 10개 단위로 실속 있게 구매할 수 있습니다.', image_url: 'minsook_detail_2.jpg', price: 5500, unit: '개', step: 10.0, badge_text: '가정용 추천', badge_class: 'badge-good', is_bite: 0, is_qty: 1 },
          { id: 5, name: '공품 오이 (못난이)', description: '조금 휘거나 크기가 일정하지 않은 등급이지만, 맛과 아삭함은 특품과 똑같습니다. 가격 부담이 적어 오이지, 오이소박이 등 대용량 조리 시 적합합니다.', image_url: 'minsook_detail_1.jpg', price: 3900, unit: 'kg', step: 1.0, badge_text: '실속형 대용량', badge_class: 'badge-value', is_bite: 0, is_qty: 0 },
          { id: 6, name: '한입 오이', description: '작고 귀여운 사이즈로 등산이나 나들이 갈 때 가방에 쏙 들어갑니다. 껍질째 그대로 한 입에 아삭아삭 씹어 먹기 편리한 민숙농장의 인기 시그니처입니다.', image_url: 'minsook_main.jpg', price: 5800, unit: 'kg', step: 0.5, badge_text: '농장 시그니처', badge_class: 'badge-signature', is_bite: 1, is_qty: 0 }
        ];
        localStorage.setItem('minsook_products', JSON.stringify(products));
      }
  ...
  function fallbackToLocalProducts() {
    const localProducts = JSON.parse(localStorage.getItem('minsook_products') || '[]');
  ...
  function renderMainPageProductCards(products) {
    ...
    products.forEach(p => {
      ...
      } else if (name.includes('상품')) {
        if (p.is_qty === 1 || p.is_qty === true || p.unit === '개') {
          goodQty = p;
        } else {
          goodKg = p;
        }
      } else if (name.includes('공품') || name.includes('못난이')) {
        valueProduct = p;
      } else {
        customProducts.push(p);
      }
    ...
    // We will build a list of cards to render
    const cardsToRender = [];

    // 1. 한입 오이 (First!)
    if (biteProduct) {
      const halfPrice = Math.round(biteProduct.price / 2);
      cardsToRender.push({
        ...
        priceHTML: `
          ...
          <span class="price-unit-exclusive">※ 1kg 환산: ${biteProduct.price.toLocaleString()}원</span>
        `
      });
    }

    // 2. 특품 오이
    if (specialKg || specialQty) {
      const pRef = specialKg || specialQty;
      let priceHTML = '';
      if (specialKg && specialQty) {
        priceHTML = `
          무게: <span class="price-val">${specialKg.price.toLocaleString()}</span>원 <span class="price-unit-inline">(1${specialKg.unit}당)</span><br>
          개수: <span class="price-val">${specialQty.price.toLocaleString()}</span>원 <span class="price-unit-inline">(${specialQty.step}${specialQty.unit}당)</span>
        `;
      } else if (specialKg) {
        priceHTML = `무게: <span class="price-val">${specialKg.price.toLocaleString()}</span>원 <span class="price-unit-inline">(1${specialKg.unit}당)</span>`;
      } else {
        priceHTML = `개수: <span class="price-val">${specialQty.price.toLocaleString()}</span>원 <span class="price-unit-inline">(${specialQty.step}${specialQty.unit}당)</span>`;
      }
      cardsToRender.push({
    ...
    // 3. 상품 오이
    if (goodKg || goodQty) {
      const pRef = goodKg || goodQty;
      let priceHTML = '';
      if (goodKg && goodQty) {
        priceHTML = `
          무게: <span class="price-val">${goodKg.price.toLocaleString()}</span>원 <span class="price-unit-inline">(1${goodKg.unit}당)</span><br>
          개수: <span class="price-val">${goodQty.price.toLocaleString()}</span>원 <span class="price-unit-inline">(${goodQty.step}${goodQty.unit}당)</span>
        `;
      } else if (goodKg) {
        priceHTML = `무게: <span class="price-val">${goodKg.price.toLocaleString()}</span>원 <span class="price-unit-inline">(1${goodKg.unit}당)</span>`;
      } else {
        priceHTML = `개수: <span class="price-val">${goodQty.price.toLocaleString()}</span>원 <span class="price-unit-inline">(${goodQty.step}${goodQty.unit}당)</span>`;
      }
      cardsToRender.push({
    ...
    customProducts.forEach(p => {
      ...
      if (units.includes('kg') && units.includes('개')) {
        ...
        priceHTML = `
          <span class="price-val">${halfPrice.toLocaleString()}</span>원 <span class="price-unit-inline">(무게 500g 1팩당)</span><br>
          <span class="price-unit-exclusive">※ 1kg 환산: ${p.price.toLocaleString()}원</span>
        `;
      } else if (p.is_qty === 1 || p.is_qty === true) {
        priceHTML = `
          <span class="price-val">${p.price.toLocaleString()}</span>원 <span class="price-unit-inline">(${p.step}${p.unit}당)</span>
        `;
      } else {
        priceHTML = `
          <span class="price-val">${p.price.toLocaleString()}</span>원 <span class="price-unit-inline">(1${p.unit}당)</span>
        `;
      }
      cardsToRender.push({
    ...
    cardsToRender.forEach((card, idx) => {
      ...
      } else if (title.includes('상품')) {
        images = sliderImagesMap['상품'];
  ...
  // Init dynamic products and calculations
